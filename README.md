...
- 数字时钟（固定展示，含日期与星期）
- 番茄钟
- 5 款皮肤（深空/森林/深海/暮色/晨雾，设置中切换，即时生效并记住选择）
- 自定义倒计时（按目标时间或时长设定，适合"距离下课/会议还有多久"场景）
- 自动定位显示实时天气（macOS 系统定位 → IP 定位 → 手动城市，三级降级，支持语音播报）
- 番茄完成次数按天记录
- 每轮专注显示一条励志金句
- 专注结束自动进入休息
- 休息结束自动切回下一轮专注待机
...
- 天气卡片默认开启，自动通过 macOS 系统定位获取位置；未授权时降级为 IP 定位。
